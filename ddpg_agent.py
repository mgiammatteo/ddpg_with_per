...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        with torch.no_grad():
            action_state = self.actor_local(state)
            action_next_state = self.actor_target(next_state)
        
        self.actor_local.train()
        
        q_next_state = self.critic_target(next_state, action_next_state).cpu().detach().numpy()
        expected_q_state = self.critic_local(state, action_state).cpu().detach().numpy()
        
        td_error = np.array(reward + GAMMA*q_next_state*(1-done)-expected_q_state)
    ...
    def learn(self, experiences, gamma):
        ...
        Params
        ======
            experiences (Tuple[torch.Tensor]): tuple of (s, a, r, s', done, index, weight) tuples 
        ...
        critic_loss = F.mse_loss(Q_expected*weights, Q_targets*weights)
        # Minimize the loss
...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, priority, done):
        """Add a new experience to memory with a priority."""
